...
float calculateSpeed(float forwardDistance, float targetSeconds, float elapsedSeconds, float minSpeed = 17) { //YOU WANT TO CHANGE THIS AS WELL
  double PercentOfTrap = 0.4;
  float speedMultiplier = 1.05;
  if (forwardDistance < 0) speedMultiplier = -1.05;

  float percentComplete = elapsedSeconds / targetSeconds;
  float delta = (fabs(forwardDistance) - minSpeed * targetSeconds) / ((1-PercentOfTrap) * targetSeconds); // diff between max and min speed
...
float calculateSpeed(float forwardDistance, float targetSeconds, float elapsedSeconds, float minSpeed = 5) {
  float speedMultiplier = 1.05;
  if (forwardDistance < 0) speedMultiplier = -1.05;
